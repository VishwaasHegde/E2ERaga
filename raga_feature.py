...
def get_raga_feat_cache(mbid, r, delta, raga_feat, all_notes, tradition):
    lim = 55


    with h5py.File('images/{}/raga_feat_cache_1.hdf5'.format(tradition), "r") as lm_file_ndms:
        ...
        for i in range(0, 60, 5):
            for j in range(5, lim + 1, 5):
                ...
                for k in range(4):
                    f = lm_file_ndms['{}_{}_{}_{}'.format(mbid, s, e, k)]
                    if len(f)==0:
                        raga_feat[s // 5, e // 5, :, k] = np.zeros(60)
                        continue
                    f1 = binary_search.get_bound_util(f, f.shape[0], r, True)
                    f2 = binary_search.get_bound_util(f, f.shape[0], r + delta, False)

                    raga_feat[s//5,e//5,:,k] = gauss_smooth_util(f2-f1, all_notes)

    return raga_feat

def gauss_smooth_util(arr, all_notes):
    smooth = 0
    for i in range(60):
