...
count=0
def encode(inputs, note_emb, note_strength, inputs_mask, n_layers, heads=4, keep_prob=0.8, d_model=128):
...
def encoder(x: tf.Tensor, *,
    ...
    for i in range(n_layers):
        x = encoder_layer(x, mask=mask, index=i, heads=heads, keep_prob=keep_prob, d_ff=d_ff)
...
def encoder_layer(x: tf.Tensor, *,
    ...
    added = x + Dropout(keep_prob)(attention_out)
    x = layer_norm(added)
    ff_out = feed_forward(x, d_model, d_ff, keep_prob)
    added = x + Dropout(keep_prob)(ff_out)
