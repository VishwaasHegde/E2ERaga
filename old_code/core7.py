...
def build_and_load_model(config, task ='raga'):
    ...
    #Tonic
    tonic_emb = get_tonic_emb(red_y,note_emb, note_dim, drop_rate_tonic)

    tonic_logits = Dense(60, activation='sigmoid', name='tonic')(tonic_emb)

    tonic_model = Model(inputs=[pitches_batch], outputs=tonic_logits)
    tonic_model.summary()
...
def get_top_notes(red_y):
    ...
    return diag_tf_4, diag_tf_3_soft
    # return diag_tf_3, diag_tf_3

def get_hist_emb(red_y, note_emb, note_dim, indices, topk, drop_rate=0.2):
    top_notes, top_notes_soft = get_top_notes(red_y)
    emb_dot = tf.reduce_mean(tf.multiply(note_emb, tf.tile(note_emb[0:1], [60, 1])), axis=1)
    emb_dot = tf.tile(tf.expand_dims(emb_dot,0), [topk,1])
    hist = tf.reduce_mean(red_y, axis=0)
    hist = min_max_scale(hist)
    top_notes_soft = min_max_scale(top_notes_soft)
    top_notes = min_max_scale(top_notes)
    hist_cc = tf.transpose(tf.stack([hist, top_notes_soft, top_notes]))
    ...
    z = MaxPool1D(pool_size=2)(z)
    z = Dropout(drop_rate)(z)
    z = Conv1D(filters=196, kernel_size=3, strides=1,padding='same', activation='relu')(z)
    z = Conv1D(filters=196, kernel_size=3, strides=1, padding='same', activation='relu')(z)
    z = BatchNormalization()(z)
    z = MaxPool1D(pool_size=2)(z)
    z = Dropout(drop_rate)(z)
...
def get_tonic_emb(red_y, note_emb, note_dim, drop_rate=0.2):
    hist_emb, top_notes = get_hist_emb(red_y, note_emb, note_dim, None, 1, drop_rate)
...
def get_raga_emb(red_y, tonic_logits, note_emb, note_dim, drop_rate=0.2):
    # rs = -5
    # re = 5
    topk = 10

    # values, indices = conv_tonic(tonic_logits)
    # values = tf.cast(values, tf.float32)
    values, indices = tf.nn.top_k(tonic_logits[0], k=topk)

    # transpose_by = tf.cast(tf.argmax(tonic_logits, axis=1)[0], tf.int32)
    # red_y = tf.roll(red_y, -transpose_by, axis=1)
    hist_emb, top_notes = get_hist_emb(red_y, note_emb, note_dim, indices, topk, drop_rate)
    ndms = get_ndms(red_y, top_notes, indices, topk, note_emb, note_dim, drop_rate)
...
def get_ndms(red_y, top_notes, indices, topk, note_emb, note_dim, drop_rate=0.6):
    ...
    matmul_1 = min_max_scale(matmul_1)
    matmul_2 = min_max_scale(matmul_2)
    matmul_3 = min_max_scale(matmul_3)
    # matmul_3 = matmul_3/tf.cast(tf.shape(red_y_am)[0], tf.float32)
    # matmul_4 = matmul_4 / tf.cast(tf.shape(red_y_am)[0], tf.float32)
    matmul = tf.stack([matmul_1, matmul_2, matmul_3], axis=2)
    ...
    z = MaxPool2D(pool_size=(2, 2), strides=None, padding='valid')(z)
    z = Dropout(drop_rate)(z)
    z = Conv2D(filters=196, kernel_size=(3, 3), strides=(1, 1), activation='relu', padding='same')(z)
    z = Conv2D(filters=196, kernel_size=(3, 3), strides=(1, 1), activation='relu', padding='same')(z)
    z = BatchNormalization()(z)
    z = MaxPool2D(pool_size=(2, 2), strides=None, padding='valid')(z)
    z = Dropout(drop_rate)(z)
...
def conv_tonic(pred_tonic):
    ...
    bces = tf.concat([bces, bces, bces], axis=-1)
    bces_1 = tf.concat([c_note, c_note, c_note], axis=-1)
    values = bces[60+bces_argmin-5:60+bces_argmin+5]
    ...
    indices = tf.range(bces_argmin-5, bces_argmin+5)
